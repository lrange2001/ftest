...
class SSHConsumer(WebsocketConsumer):
    ...
    # 处理请求参数
    def connect(self):
        # self.ssh_ip = Server.objects.get(hostname=self.hostname).ipaddress
        # self.ssh_port = Server.objects.get(hostname=self.hostname).port
    ...
    def ssh_stream(self):
        ...
        try:
            ...
            credential = Server.objects.get(hostname=self.hostname).auth_choice
            print(credential)
            username = Server.objects.get(hostname=self.hostname).username
            if credential == '1':
                '''密码处理'''
                password = Server.objects.get(hostname=self.hostname).auth_password
