  ...
  >
        <el-form :model="form" label-width="120px" ref="formRef" :rules="formRules">
          ...
          <el-input  v-model="form.username"  />
            ...
            <el-radio-group v-model="form.auth_choice" class="ml-4" style="float: left"  >
             ...
             <el-form-item label="请输入密码"  v-if="form.auth_choice == 1" prop="auth_password">
                <el-input   v-model="form.auth_password"   show-password   ></el-input>
              </el-form-item>

            <el-form-item label="请添加密钥" v-else-if="form.auth_choice == 2"  prop="auth_key">
    ...
    <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="submit" :loading="submiting">修改</el-button>
        </span>
      </template>
  </el-dialog>
</template>
...
<script>
    export default {
        ...
        beforeUpdate() {
            //当窗口打开时
            this.$axios.get('/server/NodeHostname/' + this.editdata.hostname + '/').then(res =>{
                this.form.hostname= res.data.data.hostname;
                this.form.ipaddress= res.data.data.ipaddress;
                this.form.username=res.data.data.username;
                this.form.auth_choice= res.data.data.auth_choice;
                this.form.auth_password= res.data.data.auth_password;
                this.form.auth_key=res.data.data.auth_key;
                          ...
                          if(res.data.code == 200){
                              this.$axios.post('/server/gather_node/',{'hostname':this.form.hostname}).then(rsync =>{
                                  if(rsync.data.code == 200){
                                      ...
                                      this.$message.warning('更新完成,但同步失败,详情查看控制台')
                              ...
                              this.$message.error('更新失败,详情查看控制台');
                              this.submiting = false;
